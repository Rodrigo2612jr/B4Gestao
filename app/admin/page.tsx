...
function Inner() {
  ...
  return (
    <div className="space-y-7">
      <Reveal>
        <span className="b4-eyebrow">Painel da operação</span>
        <h2 className="mt-2 text-[26px] font-bold text-b4-ink" style={{ fontFamily: DISPLAY, letterSpacing: "-0.03em" }}>Visão geral</h2>
        <p className="mt-1 text-sm text-b4-ink-2">Resumo da operação · dados em tempo real.</p>
      </Reveal>

      {/* ===== KPI row ===== */}
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        {/* Featured navy · Leads com sparkline real */}
        <Reveal delay={40}>
          <div className="b4-feature h-full p-5">
            <div className="relative">
              <div className="flex items-center gap-2 text-white/85">
                <span className="flex h-9 w-9 items-center justify-center rounded-xl bg-white/15 text-lg ring-1 ring-white/15"><HiOutlineClipboardList /></span>
              ...
              {loading || !k ? <div className="mt-4 h-9 w-16 animate-pulse rounded bg-white/20" /> : (
                <>
                  <p className="mt-2 text-4xl font-bold tabular-nums" style={{ fontFamily: DISPLAY }}><CountUp value={k.leadsTotal} /></p>
          ...
          <Card title="Conclusão de AEP" subtitle="Avaliações ergonômicas aprovadas">
            {loading ? <Skeleton h={140} /> : aepTotal === 0 ? (
              ...
              <div className="flex flex-col items-center">
                ...
                <p className="mt-1 text-xs text-b4-ink-2">{aepAprov} de {aepTotal} avaliações aprovadas</p>
              </div>
            )}
          </Card>
        </Reveal>
                ...
                <div className="space-y-1.5 text-xs">
                  {semSegments.map((s) => (
                    <div key={s.label} className="flex items-center gap-2">
                      ...
                      <span className="text-b4-ink-2">{s.label}</span>
                      <span className="ml-auto font-semibold tabular-nums text-b4-ink-3">{s.value}</span>
                    </div>
                  ))}
                </div>
              </div>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        <Reveal delay={150} className="lg:col-span-2">
          <Card title="Atividade recente" subtitle="Últimas ações na operação">
            {loading || !data ? <Skeleton h={200} /> : data.activity.length === 0 ? (
              ...
              <ul className="-mx-1 divide-y divide-b4-line">
                {data.activity.map((a, idx) => {
                  const m = ACTIVITY_META[a.type] ?? ACTIVITY_META.lead;
                  return (
                    <li key={idx} className="flex items-center gap-3 rounded-lg px-1 py-3 transition-colors hover:bg-b4-surface-2">
                      ...
                      <div className="min-w-0 flex-1">
                        <p className="truncate text-sm font-medium text-b4-ink">{a.label}</p>
                        <p className="truncate text-xs text-b4-ink-2">{a.meta}</p>
                      </div>
                      <span className="flex-shrink-0 text-xs text-b4-ink-3">{timeAgo(a.at)}</span>
                    </li>
                  );
                })}
              </ul>
        ...
        <Reveal delay={220}>
          <div className="b4-feature h-full p-6">
            <div className="relative">
              <h3 className="text-sm font-bold uppercase tracking-wide text-white/85" style={{ fontFamily: DISPLAY }}>Operação NR-1</h3>
              ...
              <Link href="/admin/companies" className="mt-6 inline-flex items-center gap-1 rounded-xl bg-white/15 px-3 py-2 text-xs font-semibold text-white ring-1 ring-white/15 transition-colors hover:bg-white/25">
...
function KpiCard({ label, value, hint, loading, icon, tone, href }: {
  ...
  const inner = (
    <div className="b4-card b4-card-hover h-full p-5">
      ...
      {loading ? <div className="mt-4 h-9 w-12 animate-pulse rounded bg-b4-surface-2" /> : (
        <>
          <p className="mt-3 text-4xl font-bold tabular-nums text-b4-ink" style={{ fontFamily: DISPLAY }}>{value != null ? <CountUp value={value} /> : "-"}</p>
          {hint && <p className="mt-1 text-xs text-b4-ink-3">{hint}</p>}
        </>
      )}
    </div>
  );
...
function Card({ title, subtitle, children }: { title: string; subtitle?: string; children: React.ReactNode }) {
  return (
    <section className="b4-card h-full p-5">
      <div className="mb-4">
        <h3 className="text-sm font-bold text-b4-ink">{title}</h3>
        {subtitle && <p className="text-xs text-b4-ink-2">{subtitle}</p>}
      </div>
      {children}
    </section>
  );
...
function MiniStat({ label, value, hint, loading }: { label: string; value?: number; hint?: string; loading?: boolean }) {
  return (
    <div className="flex items-end justify-between border-b border-white/10 pb-3 last:border-0 last:pb-0">
      ...
      {loading ? <div className="h-6 w-8 animate-pulse rounded bg-white/20" /> : <p className="text-2xl font-bold tabular-nums" style={{ fontFamily: DISPLAY }}>{value != null ? <CountUp value={value} /> : "-"}</p>}
    </div>
  );
}

function Skeleton({ h }: { h: number }) {
  return <div className="animate-pulse rounded-xl bg-b4-surface-2" style={{ height: h }} />;
}

function EmptyMini({ icon, text }: { icon: React.ReactNode; text: string }) {
  return (
    <div className="flex flex-col items-center justify-center py-10 text-center">
      <span className="flex h-12 w-12 items-center justify-center rounded-2xl bg-b4-surface-2 text-2xl text-b4-ink-3">{icon}</span>
      <p className="mt-3 text-xs text-b4-ink-3">{text}</p>
    </div>
  );
}
