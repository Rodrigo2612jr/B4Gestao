  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      ...
      <div className="b4-card space-y-4 p-6">
        <h2 className="text-lg font-semibold text-b4-ink" style={{ fontFamily: "var(--font-admin-display), system-ui, sans-serif" }}>Dados da avaliação</h2>
        <div>
          <label className="text-sm font-medium text-b4-ink-2">Título da avaliação *</label>
          <input
            value={title}
            onChange={(e) => setTitle(e.target.value)}
            placeholder="Ex: AEP · Canteiro Obra Central / 2026"
            className="mt-1 w-full rounded-xl border border-b4-line-strong bg-b4-surface px-3 py-3 text-sm text-b4-ink outline-none transition-all placeholder:text-b4-ink-3 focus:border-b4-navy focus:ring-4 focus:ring-b4-navy/12"
          />
        </div>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div>
            <label className="text-sm font-medium text-b4-ink-2">Unidade</label>
            <input
              value={unidade}
          ...
          <div>
            <label className="text-sm font-medium text-b4-ink-2">Data da avaliação</label>
            <input
              type="date"
              value={data}
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div>
            <label className="text-sm font-medium text-b4-ink-2">Cargo do avaliador</label>
            <input
              value={cargo}
          ...
          <div>
            <label className="text-sm font-medium text-b4-ink-2">Supervisor (aprovação)</label>
            <select
              value={supervisorId}
        ...
        <p className="text-xs text-b4-ink-2">Você (técnico) será o avaliador desta AEP. O supervisor poderá acompanhar e aprovar.</p>
      </div>
      ...
      <div className="flex justify-end">
        <button
          onClick={submit}
          disabled={submitting}
          className="flex h-11 items-center gap-1.5 rounded-xl bg-b4-navy px-5 text-sm font-semibold text-white shadow-[var(--b4-shadow-xs)] transition-colors hover:bg-b4-navy-deep active:scale-95 disabled:opacity-60"
        >
